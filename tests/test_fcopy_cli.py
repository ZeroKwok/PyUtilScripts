...
def test_cli_source_dir_not_exist(monkeypatch, patch_common):
    ...
    code = main()  # Should print error and return 1
    assert code == 1
...
def test_cli_copy_files_dry_run(monkeypatch, patch_common):
    ...
    monkeypatch.setattr("pyutilscripts.fcopy.read_file_list", lambda f: ["file1.txt"])
    # Patch make_actions to return actions
    monkeypatch.setattr("pyutilscripts.fcopy.make_actions", lambda args: [("c", "file1.txt", "")])
...
def test_cli_target_missing(monkeypatch, patch_common):
    ...
    monkeypatch.setattr("os.path.isdir", lambda p: True)
    code = main()  # Should print error and return
    assert code == 1
